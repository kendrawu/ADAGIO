...
# The functions that are called by sbm.jl
...
# The function that calls other functions to perform the computation
function fn_main(N, endtime, par_hh, par_community, par_disease, import_lambda, importcasenum_timeseries, nstatus, casenum0)
    ...
    # Output:
    # sbm_sol: The matrix that holds SEIR incidence of all timestep

    # Initialization
    sbm_sol = DataFrame(S=fill(0,round(Int,endtime)), E=fill(0,round(Int,endtime)), I=fill(0,round(Int,endtime)), R=fill(0,round(Int,endtime)), V=fill(0,round(Int,endtime)))
...
sbm_sol = fn_main(N, endtime, par_hh, par_community, par_disease, import_lambda, importcasenum_timeseries, nstatus, casenum0)

...
for isim in 1:nsim
    ...
    compartmentsize_keyvec_arr = fn_par_cluster2(N, par_hh, par_community, par_compartment, "compartment")
    ...
    nodes_infectious_cases = findall(x->x=='I', nstatus[:,round(Int,intermediatetime2)+1])
    compartment_arr = setdiff(potential_nodes_notrial_index, nodes_infectious_cases) # Nodes name of those key transmission vector - children - not in trial

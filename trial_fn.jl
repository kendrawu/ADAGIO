...
function fn_iteration_iRCT_MLE_tmp(nsim, soln1, nstatuts1, tstatus1, VE_true1, samplesize1, n_control1, n_treatment1, n_infectious_control1, n_infectious_treatment1, n_exposed_control1, n_exposed_treatment1, N, par_hh, par_community, par_prob, par_disease, prop_in_trial, import_lambda, casenum0, immunenum0, allocation_ratio, vac_efficacy, protection_threshold, treatment_gp, gamma_infectperiod_maxduration, trial_begintime, trial_endtime, endtime)
    ...
    allocation_ratio = fn_adapt_MLE(nsim, n_control, n_treatment, n_infectious_control, n_infectious_treatment)
    println("New optimum allocation (MLE): ", allocation_ratio)
    (tstatus, nodes_in_control, nodes_in_treatment) = fn_trialsetup_iRCT(N, par_disease, tstatus, communitysize_arr, communitynum_arr, trial_communitynum, nstatus, allocation_ratio, vac_efficacy, protection_threshold)
    (nstatus2, tstatus2, soln2, T2, R02) = fn_transmodel(nstatus, tstatus, sbm_sol, par_hh, par_community, par_prob, par_disease, hhnum_arr, trial_communitynum, communitysize_arr, communitynum_arr, importcasenum_timeseries, Gc, trial_begintime+1, endtime, endtime)
    ...
    for itern = 3:nsim
        ...
        println("New optimum allocation (MLE): ", allocation_ratio)
        (tstatus, nodes_in_control, nodes_in_treatment) = fn_trialsetup_iRCT(N, par_disease, tstatus, communitysize_arr, communitynum_arr, trial_communitynum, nstatus, allocation_ratio, vac_efficacy, protection_threshold)
        (nstatus3, tstatus3, soln3, T3, R03) = fn_transmodel(nstatus, tstatus, sbm_sol, par_hh, par_community, par_prob, par_disease, hhnum_arr, trial_communitynum, communitysize_arr, communitynum_arr, importcasenum_timeseries, Gc, trial_begintime+1, endtime, endtime)
...
function fn_iteration_cRCT_non_adpative(nsim, soln1, nstatus1, tstatus1, VE_true1, samplesize1, n_infectious_control1, n_infectious_treatment1, n_exposed_control1, n_exposed_treatment1, N, par_hh, par_community, par_prob, par_disease, prop_in_trial, import_lambda, casenum0, immunenum0, trial_communitynum, allocation_ratio, vac_efficacy, protection_threshold, treatment_gp, gamma_infectperiod_maxduration, trial_begintime, trial_endtime, endtime)
...
function fn_iteration_cRCT_MLE(nsim, soln1, nstatus1, tstatus1, VE_true1, samplesize1, n_infectious_control1, n_infectious_treatment1, n_exposed_control1, n_exposed_treatment1, N, par_hh, par_community, par_prob, par_disease, prop_in_trial, import_lambda, casenum0, immunenum0, trial_communitynum, allocation_ratio, vac_efficacy, protection_threshold, treatment_gp, gamma_infectperiod_maxduration, trial_begintime, trial_endtime, endtime)
...
function fn_iteration_cRCT_Bayes(nsim, soln1, nstatus1, tstatus1, VE_true1, samplesize1, N, par_hh, par_community, par_prob, par_disease, prop_in_trial, import_lambda, casenum0, immunenum0, trial_communitynum, allocation_ratio, vac_efficacy, protection_threshold, treatment_gp, gamma_infectperiod_maxduration, trial_begintime, trial_endtime, endtime)
...
function fn_adapt_MLE(method, n_control, n_treatment, n_infectious_control, n_infectious_treatment)
    ...
    p0u = success_control/ n_control
    p1u = success_treatment/ n_treatment
...
function fn_adapt_Bayes(itern, n_control, n_treatment, n_infectious_control, n_infectious_treatment)
    ...
    for i in 1:itern
        ...
        if soln[i] < soln_truecases
            ...
            push!(element_selected3_p0::Array{Float64,1},p1r)
            posterior = [p0r, p1r]
            rewardnum1 = rewardnum1 + 1
            if length(element_selected3_p0)==0 || length(element_selected3_p1)==0
                posterior=[p0,p1]
            end
...
function fn_trialsetup_iRCT_highrisk_tmp(N, tstatus, prop_in_trial, prop_in_highrisk, allocation_ratio, vac_efficacy, protection_threshold)
    ...
    # tstatus: The trial statuses of each individual in the population
    # prop_in_trial: Proportion of a cluster will be enrolled into the trial
    # prop_in_highrisk: Proportion of a population/ cluster are at high risk
    # allocation_ratio: Allocation ratio of the randomization trial
    # vac_efficacy: Vaccine efficacy of vaccine candidate(s)
    # protection_threshold: A value to determine if a vaccinated person will be infected if exposed
    ...
    # Enrollment size among high risk individuals
    enrolsize = ceil(Int,length(potential_nodes_notrial_index)*prop_in_trial*prop_in_highrisk)
    #println("Enrollment size = ", enrolsize)
...
function fn_iteration_iRCT_highrisk_tmp(nsim, soln1, nstatuts1, tstatus1, VE_true1, samplesize1, n_infectious_control1, n_infectious_treatment1, n_exposed_control1, n_exposed_treatment1, N, par_hh, par_community, par_prob, par_disease, prop_in_trial, prop_in_highrisk, import_lambda, casenum0, immunenum0, allocation_ratio, vac_efficacy, protection_threshold, treatment_gp, gamma_infectperiod_maxduration, trial_begintime, trial_endtime, endtime)
    ...
    (nstatus, tstatus, soln, T, R0) = fn_transmodel(nstatus, tstatus, sbm_sol, par_hh, par_community, par_prob, par_disease, hhnum_arr, communitysize_arr, communitynum_arr, importcasenum_timeseries, Gc, trial_begintime+1, trial_begintime+trial_posthighrisk, endtime)
    (tstatus, nodes_in_control, nodes_in_treatment) = fn_trialsetup_iRCT(N, tstatus, prop_in_trial, allocation_ratio, vac_efficacy, protection_threshold)
    (nstatus2, tstatus2, soln2, T2, R02) = fn_transmodel(nstatus, tstatus, sbm_sol, par_hh, par_community, par_prob, par_disease, hhnum_arr, communitysize_arr, communitynum_arr, importcasenum_timeseries, Gc, trial_begintime+trial_posthighrisk+1, endtime, endtime)
    ...
    for itern = 3:nsim
        ...
        (nstatus, tstatus, soln, T, R0) = fn_transmodel(nstatus, tstatus, sbm_sol, par_hh, par_community, par_prob, par_disease, hhnum_arr, communitysize_arr, communitynum_arr, importcasenum_timeseries, Gc, trial_begintime+1, trial_begintime+trial_posthighrisk, endtime)
        (tstatus, nodes_in_control, nodes_in_treatment) = fn_trialsetup_iRCT(N, tstatus, prop_in_trial, allocation_ratio, vac_efficacy, protection_threshold)
        (nstatus3, tstatus3, soln3, T3, R03) = fn_transmodel(nstatus, tstatus, sbm_sol, par_hh, par_community, par_prob, par_disease, hhnum_arr, communitysize_arr, communitynum_arr, importcasenum_timeseries, Gc, trial_begintime+trial_posthighrisk+1, endtime, endtime)
